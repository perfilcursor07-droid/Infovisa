...
@section('title', 'Editar Tipo de Setor')
...
<div class="container-fluid px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Editar Tipo de Setor</h1>
                <p class="text-sm text-gray-600 mt-1">Atualize as informações do tipo de setor</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form method="POST" action="{{ route('admin.configuracoes.tipo-setores.update', $tipoSetor) }}" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="codigo" 
                               name="codigo" 
                               value="{{ old('codigo', $tipoSetor->codigo) }}"
                               required
                    ...
                    <div class="flex items-center pt-6">
                        <input type="checkbox" 
                               ...
                               name="ativo" 
                               value="1"
                               {{ old('ativo', $tipoSetor->ativo) ? 'checked' : '' }}
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="descricao" 
                                  ...
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('descricao') border-red-500 @enderror">{{ old('descricao', $tipoSetor->descricao) }}</textarea>
                        @error('descricao')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @foreach($niveisAcesso as $nivel)
                        @php
                            $isChecked = old('niveis_acesso') 
                                ? in_array($nivel->value, old('niveis_acesso', [])) 
                                : ($tipoSetor->niveis_acesso && in_array($nivel->value, $tipoSetor->niveis_acesso));
                        @endphp
